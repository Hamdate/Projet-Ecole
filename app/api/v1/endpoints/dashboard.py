...
from app.models.eleves import Eleve, Parent
...
def get_dashboard(id_etablissement: int = 2, db: Session = Depends(get_db)):
    ...
    presences_jour = db.query(
        PresenceEleve.statut, func.count(PresenceEleve.id_presence)
    ).filter(
        PresenceEleve.date_presence == aujourdhui
    ).group_by(PresenceEleve.statut).all()

    presences = {"PRESENT": 0, "ABSENT": 0, "RETARD": 0, "EXCUSE": 0}
    for statut, count in presences_jour:
        presences[statut] = count

    total_recettes = db.query(func.coalesce(func.sum(Paiement.montant), 0)).filter(
